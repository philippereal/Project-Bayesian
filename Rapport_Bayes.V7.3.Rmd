...
####                     passage aux log10 pour corriger dépassement
#########################################################################
ModChoBayesReg2.1=function(y,X,g=length(y),betatilde=rep(0,dim(X)[2]),niter=100000,cr=FALSE,prt=TRUE)
{
X=as.matrix(X)
n=length(y)
p=dim(X)[2]
...
####                     passage aux log10 pour corriger dépassement
#########################################################################
ModChoBayesReg2=function(y,X,g=length(y),betatilde=rep(0,dim(X)[2]),bCalc=TRUE, niter=100000,cr=TRUE,prt=TRUE,nbest=10)
{
X=as.matrix(X)
n=length(y)
p=dim(X)[2]
...
ModChoBayesReg2(y.cat, X.cat,bCalc=FALSE,cr=TRUE)

```
```{r }
ModChoBayesReg2.1(y.cat, X.cat,cr=TRUE)
```
```{r }
ModChoBayesReg2(y.cat, X.cat,bCalc=FALSE,cr=TRUE, nbest=15,prt = TRUE)
```
...
if (prt==TRUE)
{
  ...
  cat("\n")
  cat("\n")
  print(data.frame(topNbestmodels=modtopNbest,postprobtopNbest=round(probtopNbest,4)))
  cat("\n")
  cat("\n")
}
...
```{r mod_1, echo=FALSE}
ModChoBayesReg2(y,X,g=length(y),bCalc=TRUE,cr=TRUE)
```
...
```{r mod_1, echo=FALSE}
gammaReg2<-ModChoBayesReg2(y, X,bCalc=FALSE,cr=TRUE)
...
##############################################
##Gibbs
Gibbs = function (y.var,X.var,g=length(y.var),niter = 1e4,cr=FALSE)
{
...
#X.var = cbind(1, X.var) # on ajoute une colonne de 1 pour beta_0
...
for(i in 1:niter){
  ...
  gamma.res<-cbind.data.frame(X=colnames(X.var),gamma.mean=colMeans(gamma))
  gamma.res<-gamma.res[order(-gamma.res$gamma.mean),]
  res <-cbind.data.frame(modelnumber,gamma)
  return (res)
}
...
# VÃ©rifions le mÃ©lange de la chaÃ®ne de Markov Ã  l'aide des autocorrÃ©lations.
par(mfrow=c(4,2))
for(i in 2:9) acf(as.numeric(gamma[,i]))
# AutocorrÃ©lation dÃ©croÃ®t rapidement. Pas besoin de sous-Ã©chantillonner.

# VÃ©rifions la convergence + le mÃ©lange Ã  l'aide de la trace (on utilise une moyenne glissante puisque les valeurs sont binaires).
library(zoo)
for(i in 2:nbCol) plot(rollapply(gamma[,i], width=100, FUN=mean), type="l")
