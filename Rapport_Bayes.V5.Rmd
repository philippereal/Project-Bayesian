...
```{r echo=FALSE}
## fonction pour calculer les bayes factor
...
```{r echo=FALSE}
RegBayes.Hopf = function (y,X,g=length(y),nu0=1,s20=summary(lm(y~−1+X))$sigma^2, niter = 1e4) 
{
  #nu0<−1  
...
#### Gibbs sampler
Gibbs.Hopf = function (y,X,g=length(y),niter = 1e4)
{
...
beta = RegBayes.Hopf(y,X,g=length(y),nu0=1,s20=summary(lm(y~−1+X))$sigma^2, niter = 1e4) 
sumBeta<-summary(beta)
betaMean<-colMeans(beta)
betaMean<-as.data.frame(betaMean)

Z= Gibbs.Hopf(y,X,g=length(y),niter = 1e4)
colmean(Z)
```
