...
##############################################
# Prédiction
predict.beta = function (y,X,g=length(y),niter = 1e4)
{
  ...
  for(i in 1:niter){
    ...
    ynew.b[i] = beta %*% Xnew[c(T, gamma.m[i,])] + rnorm(1, 0, sqrt(sigma2))
  }
  return(ynew.b)
}
...
```{r mod_1, echo=FALSE}
ModChoBayesReg2(y,X,g=length(y),bCalc=TRUE)
```
...
```{r mod_1, echo=FALSE}
gammaReg2<-ModChoBayesReg2(y, X,bCalc=FALSE)
```
...
#Iplot(rollapply(gamma[,16], width=50, FUN=mean), type="l")
