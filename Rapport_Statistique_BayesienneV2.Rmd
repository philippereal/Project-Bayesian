...
{
 data <- data %>% rename(Eff_Prs_l =  effectif_presents_serie_l,)   
 data <- data %>% rename(Eff_Prs_es=  effectif_presents_serie_es)       
 data <- data %>% rename(Eff_Prs_s =  effectif_presents_serie_s)          

 data <- data %>% rename(Eff_2e = effectif_de_seconde)        
 data <- data %>% rename(Eff_1e = effectif_de_premiere)
 ...
 data <- data %>% rename(Tx_Acc.brt_bac2e = taux_acces_brut_seconde_bac  )     
 data <- data %>% rename(Tx_Acc.brt_bac1e = taux_acces_brut_premiere_bac )
                         
 data <- data %>% rename(Tx_Acc.att_bac1e = taux_acces_attendu_premiere_bac)
 data <- data %>% rename(Tx_Acc.att_bac2e = taux_acces_attendu_seconde_bac)
...
La variable réponse ou la variable à expliquer est la variable : $Barre$. Qui correspond au barême ou nombre de points nécessaire pour pouvoir obtenir un poste dans un établissement scolaire.
...
On remarque aussi deux groupes de variables dinstintcs, avec des corrélations inter-groupe faibles et intra groupes fortes. 
...
On pourrait imaginer de ne conserver que les variables de type effectifs dans le premier groupe. Et de la même manière dans le second groupe, ne garder que les taux de réussite brutes, vaiables qui semblent redondantes avec celles de type taux attendus.

On remarque que le *taux_brute_de_resussite_seriel_l* (Tx_Suc.brt_l) est moins corrélée aux autres variables, et semble avoir une certaine indépendance.
...
Le modèle linéaire, tente d'expliquer les observations $(y_i)$ (input ici la variable *Barre*) par des covariables $(x_1,...,x_p)$ (les caractéristues de l'établissement scolaire) à partir du modèle suivant :
...
On va maintenat calculer explicitement la quantité : $E^{\pi}(\beta \mid y)$
...
summary(reg.lm)
```
On a "éliminé" l'intercept en centrant ou sinon avec on aurait dû utiliser la formule: y~X-1 pour pouvoir comparer. 
...
Ils donnent une idée de l'importance d'une variable. En effet on peut tester l'hypothèse $H_0=${Modèle sans la variable i} contre {Modèle complet}. Ceci pour chacune des variables.
...
On comparera le résultat obtenu avec le résultat précédent renvoyé par la fonction reprise
...
En donnant plus d'importance à la prior, on voit que certaines variables se dégagent: les 6, 7, 12, 13, 14 et 15éme.
...
Les 6ème (taux_brut_de_reussite_serie_l), 7ème (taux_brut_de_reussite_serie_es),	12ème (taux_acces_brut_seconde_bac), 13ème (taux_acces_attendu_seconde_bac), 14ème (taux_acces_brut_premiere_bac) et 15ème (taux_acces_attendu_premiere_bac) variables semblent être les plus significatives.
...
Le modèle n'incluant que la variable N°15 *taux_acces_attendu_premiere_bac* a une proba beaucoup plus importante (facteur 10). Par ailleurs elle est aussi présente dans presque tous les modèles alternatifs. On remarque aussi la variable N°17: *taux_reussite_attendu_total_series*. 
La variable N°13: *taux_acces_attendu_seconde_bac* la variable N°14: *taux_acces_brut_premiere_bac*.
On retrouve les variables qui se détachaient dans l'analyse des Bayes factor avec g=1. 

* A partir de la fonction (modifée) - ModChoBayesReg du package Bayess

Remarque: la valeur de la PostProb a été transformée aussi et n'est pas une plus une proba. Par contre le classement à partir de cette valeur reste valable (cf. annexe).
...
On retrouve exactement les mêmes 10 meilleurs modèles. Maintenant plutôt que de faire un calcul exact on va maintenant utiliser l'algoritme d'echantillonnage de Gibbs. L'idée est d'obtenir la distribution d'intérêt à partir des lois conditionnelles, plus facile à calculer. Cette algorithme est surtout intéressant lorsque le paramètre du modèle $\theta$ est de dimension >2. Ici $\theta = (\beta, \sigma)$
...
Pour chacune des variables, si on regarde précisemment on peut voir que souvent la chaine reste bloquée au même endroit pendant plusieurs itérations. Il est possible que l'algorithme ne soit pas correctment dimensionné.
...
Dans l'ordre croissant de significativité les variables N°7, 13, 14 et 15 ressortent, quelque soit le critère: R2, R2 ajusté, cp de Mallow et Bic.
Cependant comme dans la regréssion linéaire Bayésienne, la 15ème variable semble prépondérante: *taux_acces_attendu_premiere_bac*.
...
Si on regarde maintenant la valeur des coefficients du $\hat{\beta}$ celui correspondant à la vaériable N°6 est bien plus élévé et prédomine, quelque soit l'approche Bayésienne ou classique.p
...
$f(z\mid \alpha,m) = \alpha \frac{ m^\alpha}{z^{\alpha+1}}1_{[m,+\infty[}$ 
...
## Package R pour générer des réalisations d'une loi de Paréto
...
```{r GPD2, echo=FALSE, fig.height=6, fig.width=15}
par(mfrow=c(1,2))
x <- seq(0,10, by =0.05)
...
- Loi de paréto : $$f(z\mid \alpha,m) = \alpha \frac{ m^\alpha}{z^{\alpha+1}}1_{[m,+\infty[}$$ 
...
```
L'algorithme est bien dimensionné, la chaîne comme le montre le premier graphique explore bien toute la loi. L'autocorrélation décroit assez rapidement. L'histogramme des valeurs obtenues est régulier. 
...
### Convergence de l'algorithme de Metropolis-Hastings: mutations en mathématiques
...
### Convergence de l'algorithme de Metropolis-Hastings: mutations en anglais
...
```
Dans ce cas aussi, l'algorithme est bien dimensionné, la chaîne comme le montre le premier graphique explore bien toute la loi. L'autocorrélation décroit assez rapidement. L'histogramme des valeurs obtenues est régulier.  
...
On regarde la convergence de l'estimateur, qui d'après les graphiques est obtenue à partir de 10000 path au moins. 
...
```
La version par Gibbs utilsée ici, renvoi des résultats proches du calcul exact avec ModChoBayesReg.
