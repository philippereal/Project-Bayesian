...
##############################################
##Gibbs
Gibbs = function (y,X,g=length(y),niter = 1e4)
{
  nbCol = dim(X)[2]
  nbCol1 = nbCol-1
...
RegBayes.Hoff = function (y,X,g=length(y),nu0=1,s20=summary(lm(y~−1+X))$sigma^2, niter = 1e4) 
{
  nu0<−1  
...
#### Livre : A First Course in Bayesian Statistical Methods - Peter Hoff
#### Gibbs sampler
Gibbs.Hoff = function (y,X,g=length(y),niter = 1e4)
{
...
Cette définition découle de la formule de Bayes:
...
```{r echo=FALSE}
foo2 = zlm(Barre ~ ., data=data.mutations)
summary(foo2)
```


```{r include=FALSE}
model <- stan_glm(Barre ~ ., family = gaussian(),prior = normal(),data=data.mutations)
```
...
fit.bayeslm <- bayeslm(Barre ~ ., data=data.mutations, seed=300732, iter=5000) #iter=10000
print(fit.bayeslm, pars=c("beta", "sigma"), digits=4)
```
...
A partir de la méthoide vue en TP
...
```{r echo=FALSE}
gamma = Gibbs(y,X,g=length(y),niter = 1e4)
```
